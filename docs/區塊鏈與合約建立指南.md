...
本專案共有五個核心合約，部署於 `task-reward-vault/contracts/`：

| 合約 | 職責 | 主要呼叫者 |
| :--- | :--- | :--- |
| `IdentityNFT.sol` | 身份 SBT（Soulbound Token）— 自建 eKYC / 自然人 KYC 通過後鑄造 | Go OPERATOR key |
| `PropertyRegistry.sol` | 物件憑證 + 強屋主驗證 + 共同持有 + 揭露義務 hash | 用戶 MetaMask（登記）+ Go OPERATOR key（驗證） |
| `AgencyRegistry.sol` | 委託授權 + 合約條款（服務費/期限/違約金）+ 解約管制 | 用戶 MetaMask（授權/撤銷）+ Go OPERATOR key（鎖定/緊急） |
| `ListingStakeVault.sol` | 刊登行為押金（ETH/USDC/USDT）+ 服務費管理 | 用戶 MetaMask（鎖倉/付費）+ Go OPERATOR key（釋放/沒收） |
| `CaseTracker.sol` | 案件狀態機（租屋/買賣）+ 糾紛處理 + 仲介引入足跡 | Go OPERATOR key |

**Operator only 的設計原因**：KYC mint、物件驗證、案件推進等為平台自動化操作，由後端 Go-service 持有的固定 Operator 錢包代為執行，所有操作公開可在鏈上追溯。

> **舊合約（TaskRewardVault 系列）**：保留於目錄中，供原任務追蹤功能使用，不影響房屋平台。
...
task-reward-vault/
├── contracts/
│   ├── IdentityNFT.sol          # 身份 SBT
│   ├── PropertyRegistry.sol     # 物件憑證
│   ├── AgencyRegistry.sol       # 委託授權
│   ├── ListingStakeVault.sol    # 刊登押金
│   ├── CaseTracker.sol          # 案件狀態機
│   └── (TaskRewardVault.sol 等舊合約保留)
├── scripts/
│   └── deployHousePlatform.ts   # 房屋平台一鍵部署腳本
├── hardhat.config.ts
├── .env.example
...
| 欄位 | 必填 | 說明 |
| :--- | :--- | :--- |
| `SEPOLIA_RPC_URL` | ✅ | Alchemy 或 Infura 的 HTTPS endpoint |
| `SEPOLIA_PRIVATE_KEY` | ✅ | 部署者私鑰（**不含 `0x` 前綴**，64 位 hex） |
| `TREASURY_ADDRESS` | ✅ | 平台金庫錢包地址（TREASURY_ROLE，服務費收款） |
| `OPERATOR_ADDRESS` | 選填 | Go-service OPERATOR 錢包地址；**不填則部署者自動成為 OPERATOR** |
| `USDC_ADDRESS` | 選填 | USDC 合約地址（Sepolia：`0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238`）；不填則平台不支援 USDC |
| `USDT_ADDRESS` | 選填 | USDT 合約地址（Sepolia 無官方版，測試可留空）；不填則平台不支援 USDT |

> **注意：**
> - `SEPOLIA_PRIVATE_KEY` 格式**不含 `0x` 前綴**，與 go-service 的 `APP_PLATFORM_OPERATOR_PRIVATE_KEY`（含 `0x`）格式不同。
> - `OPERATOR_ADDRESS` 應與 go-service 的 `APP_PLATFORM_OPERATOR_PRIVATE_KEY` 對應的地址一致。
> - **沒有** `ADMIN_ADDRESS` 這個欄位：部署者（deployer）自動成為所有合約的 `DEFAULT_ADMIN_ROLE`，不需另外設定。
...
# 部署房屋平台合約至 Sepolia 測試網
npm run deploy:house-platform:sepolia
```

部署成功後，終端機會輸出 5 個合約地址（格式如下）：

```
=== House Platform Deployment ===
Deployer  : 0x...
Operator  : 0x...
Treasury  : 0x...
...
...
1. IdentityNFT         : 0x...
2. PropertyRegistry    : 0x...
3. AgencyRegistry      : 0x...
4. ListingStakeVault   : 0x...
5. CaseTracker         : 0x...
...
**直接複製最後 5 行貼入 `go-service/.env`**（變數名已對齊，可直接覆蓋）。
...
| `APP_PLATFORM_TREASURY_ADDRESS` | 收取手續費的錢包地址（TREASURY_ROLE） | `0x你的地址` |
...
| `IDENTITY_NFT_ADDRESS` | IdentityNFT 合約地址 | `0x...` |
| `PROPERTY_REGISTRY_ADDRESS` | PropertyRegistry 合約地址 | `0x...` |
| `AGENCY_REGISTRY_ADDRESS` | AgencyRegistry 合約地址 | `0x...` |
| `LISTING_STAKE_VAULT_ADDRESS` | ListingStakeVault 合約地址 | `0x...` |
| `CASE_TRACKER_ADDRESS` | CaseTracker 合約地址 | `0x...` |
