...
const PROPERTY_COMPLETENESS_LABEL: Record<PropertyCompletenessStatus, string> = {
    BASIC_CREATED: "物件已建立",
    DISCLOSURE_REQUIRED: "待補揭露資料",
    WARRANTY_REQUIRED: "待補保證資料",
    SNAPSHOT_READY: "快照已準備",
    READY_FOR_LISTING: "物件可刊登",
};
...
function setupLabel(listing: Listing) {
    ...
    return listing.setup_status === "READY" ? "刊登資料已完成" : "待補刊登資料";
}

function propertyLabel(listing: Listing) {
    if (!listing.property_id) return "尚未綁定物件";
...
export default function MyListingsPage() {
    ...
    useEffect(() => {
        getMyListings()
            .then(setItems)
            .catch((err: unknown) => setError(err instanceof Error ? err.message : "讀取我的物件失敗"))
            ...
            <main className="mx-auto flex w-full max-w-[1280px] flex-col gap-8 px-6 py-12 md:px-12">
                <header className="flex flex-col gap-4">
                    ...
                    <p className="max-w-3xl text-sm leading-[1.8] text-on-surface-variant">
                        管理房東物件、草稿、出租/出售明細與發布狀態。待補資料的草稿會停在這裡，補齊後才能公開刊登。
                    </p>
                </header>

                <section className="grid gap-4 md:grid-cols-4">
                    {[
                        ["待補資料", counts.incomplete],
                        ["可發布草稿", counts.ready],
                        ["已上架", counts.active],
                        ["物件可刊登", counts.propertyReady],
                    ].map(([label, value]) => (
                ...
                {loading ? (
                    ...
                    <section className="rounded-2xl border border-outline-variant/15 bg-surface-container-lowest p-10 text-center">
                        <h2 className="text-2xl font-bold text-on-surface">目前沒有物件草稿</h2>
                        <p className="mt-2 text-sm text-on-surface-variant">啟用房東身分或新增刊登後，會在這裡管理物件資料、出租/出售明細和發布狀態。</p>
                    </section>
                ) : (
                    <section className="grid gap-4">
                        {items.map((item) => (
                            <article key={item.id} className="cursor-pointer rounded-2xl border border-outline-variant/15 bg-surface-container-lowest p-6 transition-transform hover:-translate-y-0.5" onClick={() => navigate(`/my/listings/${item.id}`)}>
                                    ...
                                    <div>
                                        <div className="mb-3 flex flex-wrap gap-2">
                                            ...
                                            {item.draft_origin === "OWNER_ACTIVATION" ? (
                                                <span className="rounded-full bg-surface-container-low px-3 py-1 text-xs font-semibold text-on-surface-variant">房東啟用草稿</span>
                                            ) : null}
                                        </div>
                                        <h2 className="text-xl font-bold text-on-surface">{item.title || "未命名刊登草稿"}</h2>
                                        <p className="mt-1 text-sm text-on-surface-variant">{item.address || "地址未設定"}</p>
                                        {item.property?.disclosure_hash ? <p className="mt-2 text-xs text-on-surface-variant">Disclosure hash: {item.property.disclosure_hash.slice(0, 16)}...</p> : null}
                                    </div>
