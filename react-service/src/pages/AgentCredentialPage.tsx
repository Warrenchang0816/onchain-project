...
const CREDENTIAL_TYPE: CredentialType = "AGENT";

const AGENT_FIELDS = [
    { key: "holderName", label: "執業姓名", placeholder: "請填寫與證照一致的姓名" },
    { key: "licenseNumber", label: "證照或登錄字號", placeholder: "例如 ABC123456" },
    { key: "brokerageName", label: "所屬公司或品牌", placeholder: "請填寫目前執業的房仲品牌或公司" },
];
        ...
        const load = async () => {
            try {
                ...
                setState({
                    loading: false,
                    authenticated: true,
                    error: error instanceof Error ? error.message : "無法載入仲介身份頁面。",
                });
            }
        };
            ...
            <main className="mx-auto flex w-full max-w-[1080px] flex-col gap-8 px-6 py-12 md:px-12 md:py-16">
                ...
                {state.authenticated && !isVerified ? (
                    <section className="rounded-[28px] border border-outline-variant/15 bg-surface-container-lowest p-8">
                        ...
                        <p className="mt-3 text-sm leading-[1.8] text-on-surface-variant">
                            仲介身份認證建立在自然人 KYC 驗證之上。先完成 KYC，之後才能送出仲介身份申請。
                        </p>
                ...
                {state.authenticated && isVerified ? (
                    <CredentialApplicationShell
                        ...
                        title="仲介身份認證"
                        description="上傳證照、登錄或執業相關文件後，系統會先走智能審核；如果你不採用智能結果，也可以改送人工審核。審核通過後是否啟用 NFT 憑證，由你自己決定。"
