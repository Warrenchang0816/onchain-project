...
func NewTaskRewardVaultService() (TaskRewardVaultService, error) {
	...
	log.Printf("[TaskRewardVaultService] operator address : %s", operatorAddress.Hex())
	log.Printf("[TaskRewardVaultService] vault contract   : %s", cfg.RewardVaultAddress)
	log.Printf("[TaskRewardVaultService] chain ID         : %d", cfg.ChainID)
	log.Printf("[TaskRewardVaultService] ⚠️  operator must hold OPERATOR_ROLE on the vault contract.")
	log.Printf("[TaskRewardVaultService] ⚠️  verify on Etherscan: https://sepolia.etherscan.io/address/%s#readContract → hasRole(OPERATOR_ROLE, <operator address>)", cfg.RewardVaultAddress)
